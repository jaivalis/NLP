...
//        args = new String[]{"-path", "./toy-data", "-scoring-mode"}; // Part 1
        args = new String[]{"-path", "./data", "-lossless-binarization"}; // Part 2.1
        ...
        int maxTrainLength = 10;  // use if for time/memory constraints cannot use the default setting
        //int maxTestLength = 40; // this is the standard setting (e.g., to compare with the previous work)
        int maxTestLength = 20;  // use if for time/memory constraints cannot use the default setting int
        ...
        if (testMode.equalsIgnoreCase("validate")) {
            ...
            testTrees = readTrees(basePath, 2200, 2201, maxTestLength);
